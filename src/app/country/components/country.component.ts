...
export class CountryComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.country) {
      ...
      if (country) {
        const headerVersion: string = this.countryConfigService.isFeatureEnabled(FeatureType.COUNTRY_HEADER, country) ?
          'v' + this.countryConfigService.getFeatureVersion(FeatureType.COUNTRY_HEADER, country).toString() : 'UNAVAILABLE';
        const contentVersion: string = this.countryConfigService.isFeatureEnabled(FeatureType.COUNTRY_CONTENT, country) ?
          'v' + this.countryConfigService.getFeatureVersion(FeatureType.COUNTRY_CONTENT, country).toString() : 'UNAVAILABLE';
